...
def weatherapifunction(myTimer: func.TimerRequest) -> None:
    ...
    EVENT_HUB_NAMESPACE = "evnthb-weather-streaming.servicebus.windows.net"
    ...
    def get_weather_alerts(base_url, api_key, location):
        weather_alerts_url = f"{base_url}alerts.json"
        params= {
            'key': api_key,
            'q': location,
            'alerts': 'yes',
        }
        response=requests.get(weather_alerts_url, params=params)
        return handle_response(response)

    # Flatten and merge data
    def flatten_data(current_weather, forecast_weather, weather_alerts):
        location_data = current_weather.get("location", {})
        current = current_weather.get("current", {})
        condition = current.get("condition", {})
        air_quality = current.get("air_quality", {})
        forecast = forecast_weather.get("forecast", {}).get("forecastday", [])
        alert_list = weather_alerts.get("alerts", {}).get("alert", [])
    ...
    def fetch_weather_data():
        ...
        VAULT_URL = "https://kyvlt-weather-streaming.vault.azure.net/"
        ...
        alerts=get_weather_alerts(base_url, weatherapikey, location)

        # Flatten and merge data
        merged_data = flatten_data(current_weather, forecast_weather, alerts)
        
        # Sending the weather data to the event hub
